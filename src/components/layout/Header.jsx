...
const SOUND_KEY = "thejas.sys.sound";
...
const MENUS = [
  {
    ...
    items: [
      ...
      { label: profile.resumeUrl ? "Resume.pdf" : "Resume coming soon", href: profile.resumeUrl, download: true },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "View source", href: profile.repositoryUrl },
    ],
  },
  {
    label: "View",
  ...
  },
  {
    ...
    items: [
      ...
      { label: "About THEJAS.SYS", to: "/about" },
    ],
  },
];
    ...
    (item) => {
      if (item.theme) setTheme(item.theme);
        ...
        try {
          ...
          localStorage.setItem("thejas.sys.synthwave", on ? "on" : "off");
        } catch {}
      } else if (item.action === "copy-email") {
        if (profile.email) navigator.clipboard?.writeText(profile.email).catch(() => {});
                    ...
                    return (
                      <li key={i} role="none">
                        <button
                          role="menuitem"
                          type="button"
                          disabled={("href" in item && !item.href) || (item.action === "copy-email" && !profile.email)}
